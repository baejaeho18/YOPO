...
output_file = "/yopo-artifact/data/dataset/from_adflush/modified_features_target_all_adflush.csv"
target_url_file = "/yopo-artifact/scripts/perturb_html/perturbed_url_{}.csv".format(attack_option)
csv_directory = "/yopo-artifact/WebGraph/result_adflush/merged_features_with_labelled.csv"
orig_target_file = "/yopo-artifact/data/dataset/from_adflush/target_features_rf_full.csv"
# Note that AdFlush features are based on the WebGraph features
...
def find_row_by_values(csv_file, first_column_value, second_column_value, third_column_value):
    try:
        data = pd.read_csv(csv_file, skip_blank_lines=True)
        row = data[(data.iloc[:, 0] == first_column_value) & (data.iloc[:, 2] == third_column_value)]
        if not row.empty:
            return row.iloc[0]
        else:
            return None
