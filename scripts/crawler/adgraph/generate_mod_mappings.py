...
def make_mod_mapping():
    ...
    dir_path = "/yopo-artifact/data/rendering_stream/modified_html_adgraph"
    csv_file_path = "/yopo-artifact/data/rendering_stream/map_local_list_mod_final.csv"
    new_csv_file_path = "/yopo-artifact/data/rendering_stream/mod_mappings_adgraph/map_mod_0.csv"
    
    filenames, urlnames = get_filenames_in_directory(dir_path)
    create_dict(csv_file_path)
    
    lines = []
    prefix = "/yopo-artifact/data/rendering_stream/modified_html_adgraph/"
    for fname in enumerate(filenames):
        key = fname[1].split("_URL")[0]
...
def split_csv():
    ...
    while(True):
        ...
        # original_csv_file = '/yopo-artifact/data/rendering_stream/mod_mappings_webgraph/map_mod_{}.csv'.format(turn)
        ...
        with open('/yopo-artifact/data/rendering_stream/mod_mappings_adgraph/map_mod_{}.csv'.format(turn), 'w', newline='') as remaining_file:
        ...
        with open('/yopo-artifact/data/rendering_stream/mod_mappings_adgraph/map_mod_{}.csv'.format(turn + 1), 'w', newline='') as unique_file:
