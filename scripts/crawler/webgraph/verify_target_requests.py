...
while True:
    if args.oversampled:
        target_csv = "/yopo-artifact/data/dataset/from_webgraph/target_features_rf_oversampled.csv"
    else:
    ...
    domain = ""
    url = ""
    perturbed_url_file = open('/yopo-artifact/scripts/crawler/webgraph/perturbed_url_for_target_webgraph.csv', 'w', newline='')
    ...
    if args.oversampled:
        ...
        try:
            ...
            # if wrong requests are included
            print(f"An error occurred: {e}")
            print("Rerunning the script from the beginning...\n")

        # Read the second CSV file and keep only the rows corresponding to the filtered indices
        df_oversampled = pd.read_csv('/yopo-artifact/data/dataset/from_webgraph/target_features_rf_oversampled.csv')
        filtered_df = df_oversampled.iloc[filtered_indices]
        filtered_df.to_csv('/yopo-artifact/data/dataset/from_webgraph/target_features_rf_full.csv', index=False)
        ...
        filtered_df_excluded.to_csv('/yopo-artifact/data/dataset/from_webgraph/target_features_rf.csv', index=False)
    break
