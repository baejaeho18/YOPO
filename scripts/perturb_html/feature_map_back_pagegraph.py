...
from perturb_html_pagegraph import featureMapbacks
import argparse
...
def compute_diff():
    ...
    for (i, value) in enumerate(delta[1]):
        ...
        if feat_name in feature_names_pagegraph.FEATURES_TO_PERTURBE_BINARY:
            value = int(value)
            if value == 0:
                value = -1
    ...
    # ordering diff
    specific_order = ["FEATURE_URL_LENGTH", "FEATURE_SEMICOLON_IN_QUERY", "FEATURE_IN_DEGREE", "FEATURE_OUT_DEGREE", "FEATURE_IN_OUT_DEGREE", "FEATURE_PARENT_IN_DEGREE", "FEATURE_PARENT_OUT_DEGREE", "FEATURE_PARENT_IN_OUT_DEGREE", "FEATURE_AVERAGE_DEGREE_CONNECTIVITY", "FEATURE_PARENT_MODIFIED_BY_SCRIPT", "FEATURE_MODIFIED_BY_SCRIPT"]
...
with open(target_csv, "r") as f_target:
    ...
    for line in f_target:
        ...
        try:
            ...
            csv_writer.writerow([orig_url, f'"{url}"', "{}_{}.html".format(domain, domain_counts[domain])])
