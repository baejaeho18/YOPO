...
def count_code_statistics(filename, timeout=None):
        ...
        try:
            ...
            if astresult==1:
                with open(filename +'.txt','r', encoding="UTF8") as astf:
                    ...
                    ast=parseresult['ast']
                    _, _, ident_len, ident_count = treewalk_featext(ast)
                    tot_line_num = (len(source_code.split('\n')) or 1)
                    tot_char_num = sum(len(line) for line in source_code.split('\n'))
                    js_clean=re.sub(
                        r"\"[^\"]*\.?[^\"]*\"|'[^']*\.?[^']*'|\d+\.\d+|\/\/[^\n]*|\/\*.*?\*\/",
                        "", 
                        source_code, 
    ...
    return ident_len, ident_count, tot_line_num, tot_char_num, dot_count, all_brackets_count
...
# for adflush
def ModifyAvgCharPerLine(delta, js_fname):
    global soup, tot_line_num, tot_char_num
    
    char_per_line_goal = (tot_char_num / tot_line_num) + delta
    to_increase = int(tot_char_num - (char_per_line_goal * tot_line_num)) / char_per_line_goal
    ...
    for i in range(to_increase):
        modified_js_content = modified_js_content + additional_code
...
def ModifyBracketToDot(delta, js_fname):
    global soup, all_brackets_count, dot_count
    
    dot_bracket_goal = (all_brackets_count / dot_count) + delta
    # to_increase = (all_brackets_count - (dot_bracket_goal * dot_count)) / dot_bracket_goal
    to_increase = (dot_bracket_goal) * dot_count - all_brackets_count
    
    print("##")
    print("dot_bracket_goal", dot_bracket_goal)
    print("to_increase", to_increase)
    ...
    if to_increase == 0:
        ...
        modified_js_content += """var yopo1234 = ["""
        new_delta = int(to_increase / 2)
        additional_code = """[],"""
...
def ModifyAvgIdentifier(delta, js_fname):
    global soup, ident_len, ident_count

    ident_len_goal = (ident_count / ident_len) + delta
    to_increase = int((ident_count - (ident_len_goal * ident_len)) / ident_len_goal)
    ...
    modified_js_content = js_content + "var q"
    for i in range(to_increase):
        modified_js_content = modified_js_content + additional_code
    modified_js_content += " = 0;"
...
# for n-gram
def Incread_ng_0_0_2(delta, js_fname, ngram_len):
    global soup
    ...
    if delta <= 0:
        ...
        additional_code = """
yopo0702 =[btoa("1"), btoa("1"), ];
        """
    new_delta = int(delta * ngram_len)    
    modified_js_content = js_content
    for i in range(new_delta):
        modified_js_content = modified_js_content + additional_code
...
def Incread_ng_0_15_15(delta, js_fname, ngram_len):
    global soup
    ...
    if delta <= 0:
        ...
        additional_code = """yopo0702 =[btoa("1"), open, open];\n"""
...
def Incread_ng_2_13_2(delta, js_fname, ngram_len):
    global soup
    ...
    if delta <= 0:
        ...
        additional_code = """try { yopo1234 = 1;
...
def Incread_ng_15_0_3(delta, js_fname, ngram_len):
    global soup

    with open(js_fname, 'r') as file:
        js_content = file.read()
    
    js_content += "function yopoFunc() {}"
    
    if delta <= 0:
        return
    elif delta > 0:
        additional_code = """yopo0702 = [open, yopoFunc(), "A"];\n"""
...
def Incread_ng_15_0_15(delta, js_fname, ngram_len):
    global soup
    ...
    if delta <= 0:
        ...
        additional_code = """yopo0702 =[open, yopoFunc(), open];\n"""
...
def Incread_ng_15_15_15(delta, js_fname, ngram_len):
    global soup
    ...
    if delta <= 0:
        ...
        additional_code = """yopo0702 =[open, open, open];\n"""
...
def featureMapbacks(name, html, url, _js_fname, delta=None, domain=None, strategy="Centralized", candidate=None, qs="", ngram_len=1):
    global deltaNodes, soup, requestURL, baseURL, modifiedRequestURL, candidate_tags, query_strings, js_fname, ident_len, ident_count, tot_line_num, tot_char_num, dot_count, all_brackets_count, parsing_success
    deltaNodes = 0
    soup = html
    requestURL = url
    ...
    # for adflush
    elif name == "brackettodot":
        try:
            if js_fname != "":
                _ident_len, _ident_count, _tot_line_num, _tot_char_num, _dot_count, _all_brackets_count = count_code_statistics(js_fname, 180)
                ident_len = _ident_len
                ident_count = _ident_count
                tot_line_num = _tot_line_num
                tot_char_num = _tot_char_num
                dot_count = _dot_count
                all_brackets_count = _all_brackets_count
                
                ModifyBracketToDot(delta, js_fname)
        ...
        try:
            if js_fname != "" and parsing_success:
                ModifyAvgIdentifier(delta, js_fname)
        except Exception as err:
            print(err)
            pass
    elif name == "avg_charperline" and parsing_success:
        try:
            if js_fname != "":
                ModifyAvgCharPerLine(delta, js_fname)
        except:
            pass
        ...
        try:
            if js_fname != "":
                Incread_ng_0_0_2(delta, js_fname, ngram_len)
        except:
            pass
    elif name == "ng_0_15_15":
        try:
            if js_fname != "":
                Incread_ng_0_15_15(delta, js_fname, ngram_len)
        except Exception as err:
            print(err)
            pass
    elif name == "ng_2_13_2":
        try:
            if js_fname != "":
                Incread_ng_2_13_2(delta, js_fname, ngram_len)
        except:
            pass
    elif name == "ng_15_0_3":
        try:
            if js_fname != "":
                Incread_ng_15_0_3(delta, js_fname, ngram_len)
        except:
            pass
    elif name == "ng_15_0_15":
        try:
            if js_fname != "":
                Incread_ng_15_0_15(delta, js_fname, ngram_len)
        except:
            pass
    elif name == "ng_15_15_15":
        try:
            if js_fname != "":
                Incread_ng_15_15_15(delta, js_fname, ngram_len)
        except:
            pass
