...
# shine
def ModifyFirstParentAsync(add):
    global modifiedRequestURL
    tag = soup.find(BSFilterByAnyString)
    attr = getAttrByURL(tag)
    
    if add == -1:
        if 'async' in tag.attrs:
            del tag['async']
            
    elif add == 1:
        if 'async' not in tag.attrs:
            tag['async'] = None

# shine
def ModifyFirstParentDefer(add):
    global modifiedRequestURL
    tag = soup.find(BSFilterByAnyString)
    attr = getAttrByURL(tag)
    
    if add == -1:
        if 'defer' in tag.attrs:
            del tag['defer']
            
    elif add == 1:
        if 'defer' not in tag.attrs:
            tag['defer'] = None
...
def wrapNodes(layers, tagName="span"):
    global deltaNodes, soup, first_parent_wrap
    if layers < 0:
        print("wrapNodes with layers < 0!")
        return
    for i in range(layers):
        if i == layers - 1:
            tagName = first_parent_wrap
        elif i == layers - 2:
            tagName = first_parent_wrap
        tag = soup.find(BSFilterByAnyString)
        if tag is None:
...
def featureMapbacks(name, html, url, first_tag, delta=None, domain=None, strategy="Centralized", candidate=None, qs=""):
    ...
    elif name == "FEATURE_INBOUND_OUTBOUND_CONNECTIONS" or name == "FEATURE_OUTBOUND_CONNECTIONS":
        try:
            if delta != 0:
    ...
    # shine
    elif name == "FEATURE_FIRST_PARENT_ASYNC":
        try:
            ModifyFirstParentAsync(delta)
        except:
            pass

    # shine
    elif name == "FEATURE_FIRST_PARENT_DEFER":
        try:
            ModifyFirstParentDefer(delta)
        except:
            pass
    
    # shine
    elif name == "FEATURE_FIRST_PARENT_ATTR_ADDED_BY_SCRIPT":
        try:
            if delta > 0:
    ...
    # shine
    elif name == "FEATURE_FIRST_PARENT_ATTR_MODIFIED_BY_SCRIPT":
        try:
            if delta > 0:
                #TODO: implement adding function
                pass
